...
This whitepaper introduces **_Actors_**, a new kind of serverless programs for general-purpose language-agnostic computing and automation jobs (also known as agents, functions, or apps). The main goal for Actors is to make it easy for developers to build and ship reusable software tools, which are also easy to run and integrate by others. For example, Actors are useful for building web scrapers, crawlers, automations, and AI agents.
...
The output object is similar to the input object, as it contains properties and values. For example, for the `bob/screenshot-taker` Actor, the output object can look like this:
...
The goal of this whitepaper is to introduce the Actor philosophy and programming model to the public, to receive feedback, and to open the way to making Actors an open standard. To create an open standard, however, there is more work, including:
